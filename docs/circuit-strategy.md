...
> **Implementation note (2026-09-14):** the underlying escrow mechanics described above — lock a stake at creation, match it at accept, pay the winner the full pot on completion, refund both stakes on a voided dispute — are built and live (`prisma.EscrowType.STAKE`/`STAKE_PAYOUT`, `src/lib/matches.ts`'s `settleBattleOnComplete` and `ruleDispute`'s void branch, the `/api/battles` create/accept/cancel routes). This is explicitly the *mechanism* only, by product decision, not a claim that staked Battles are launch-ready: none of this section's guardrails exist yet — no per-account stake limits, no collusion/pattern detection across repeat-loss pairs, no self-exclusion/cool-off tooling, and no per-market legal review. Treat this as an internal/testing capability until that work happens, not as the V2 ship described here.

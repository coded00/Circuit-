 ...
 *
 * Moved here verbatim from `/ladder` when the Circuit Video Feed took over
 * that route as the primary nav's new "Feed" destination — this page's
 * own content/behavior is unchanged, only its URL and self-links (below)
 * moved from `/ladder` to `/leaderboard`. Reached from the homepage
 * "Leaderboard" card's "View All" link.
 *
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-6 py-8 sm:px-8 sm:py-10">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="tabs">
          <Link
            href={`/leaderboard${game ? `?game=${encodeURIComponent(game)}` : ""}`}
            className={`tab ${!isMonth && !isFriends ? "tab-active" : ""}`}
          >
            Global
          </Link>
          <Link
            href={`/leaderboard?scope=month${game ? `&game=${encodeURIComponent(game)}` : ""}`}
          ...
          {user ? (
            <Link
              href={`/leaderboard?scope=friends${game ? `&game=${encodeURIComponent(game)}` : ""}`}

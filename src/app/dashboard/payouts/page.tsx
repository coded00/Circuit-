    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card grid grid-cols-2 gap-4">
        <div>
          ...
          <div className="text-lg font-semibold">{formatNaira(totalCollected)}</div>
        </div>
        <div>
          <div className="text-xs text-muted">Total refunded</div>
          <div className="text-lg font-semibold">{formatNaira(totalRefunded)}</div>
        </div>
      </div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.tournament.id} className="border-t border-border">
                  ...
                  <td className="px-4 py-3 text-right">{formatNaira(row.collected)}</td>
                  <td className="px-4 py-3 text-right">{formatNaira(row.refunded)}</td>
